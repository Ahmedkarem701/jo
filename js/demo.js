...
addfield.onclick = function (){
	...
	newField.setAttribute('class', 'form-control input-default');
	newField.setAttribute('placeholder', 'Enter your Option');
	options.appendChild(newField);
}

deletefield.onclick = function (){
	var input_tag = options.getElementsByTagName('input');
	if(input_tag.length > 2){
		options.removeChild(input_tag[(input_tag.length) -1]);
	}
}